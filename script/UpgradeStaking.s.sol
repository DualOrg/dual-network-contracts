...
///         `StakingUpgrade`, running `reinitializePermit()` atomically so the
...
contract UpgradeStaking is Script {
    function run() external {
        ...
        console2.log("=== Staking Upgrade (v1 -> StakingUpgrade) ===");
        ...
        StakingUpgrade(payable(proxy)).upgradeToAndCall(
            address(newImpl), abi.encodeCall(StakingUpgrade.reinitializePermit, ())
        );

        vm.stopBroadcast();

        // Post-upgrade readback.
        StakingUpgrade s = StakingUpgrade(payable(proxy));
